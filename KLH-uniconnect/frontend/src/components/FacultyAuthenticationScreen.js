...
const FacultyAuthenticationScreen = ({ onBack = () => {}, onAuthenticated = () => {} }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    const endpoint = `${API_BASE}/api/auth/faculty/${isSignIn ? 'sign-in' : 'sign-up'}`;
    ...
    try {
      ...
      localStorage.setItem('klhFacultyId', body.facultyId || body.id);
      onAuthenticated(formValues.email, body.facultyId || body.id);
    } catch (error) {
      setStatus('');
  ...
  const headerSubtitle = isSignIn
    ? 'Sign in with your KLH email to access the faculty portal'
    : 'Create your KLH faculty account to start using UniConnect';
...
export default FacultyAuthenticationScreen;
