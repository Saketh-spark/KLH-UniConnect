...
const statsCards = [
  { labelKey: 'stats.classesManaged', value: 4, descKey: 'stats.classesManagedDesc', gradient: 'from-blue-200 via-blue-100 to-blue-50' },
  { labelKey: 'stats.studentsEnrolled', value: 120, descKey: 'stats.studentsEnrolledDesc', gradient: 'from-emerald-200 via-emerald-100 to-emerald-50' },
  { labelKey: 'stats.pendingReviews', value: 15, descKey: 'stats.pendingReviewsDesc', gradient: 'from-purple-200 via-purple-100 to-purple-50' },
  { labelKey: 'stats.attendanceRate', value: 87, suffix: '%', descKey: 'stats.attendanceRateDesc', gradient: 'from-amber-200 via-amber-100 to-amber-50' },
];

const easeOutCubic = (progress) => 1 - Math.pow(1 - progress, 3);

const FacultyDashboard = ({ email = '', onLogout = () => {}, onModuleSelect = () => {} }) => {
  const { t } = useLanguage();
        ...
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs uppercase tracking-[0.5em] text-slate-400">{t('dashboard.faculty')}</p>
            </div>
          </div>
          <div className="flex items-center gap-6 text-sm">
            <div className="text-right">
              <p className="text-[0.6rem] uppercase tracking-[0.5em] text-slate-400">{t('dashboard.role')}</p>
              <p className="text-base font-semibold text-slate-900">{t('dashboard.faculty')}</p>
            </div>
            <div className="text-right">
              <p className="text-[0.6rem] uppercase tracking-[0.5em] text-slate-400">{t('dashboard.emailLabel')}</p>
              <p className="text-base font-semibold text-slate-900">{email || 'faculty@klh.edu.in'}</p>
            </div>
            <button
              type="button"
              onClick={onLogout}
